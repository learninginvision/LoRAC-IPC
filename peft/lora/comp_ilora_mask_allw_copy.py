...
class CompILoraPool(nn.Module):
    ...
    def cal_delta_w(self, device, task_id=-1, depth_id=-1, train=False, position='qkv'):
        ...
        if train:
            ...
            if position == 'qkv':
                ...
                self.q_lora_W[depth_id] = q
                self.k_lora_W[depth_id] = k
                self.v_lora_W[depth_id] = v
    ...
    def get_all_w(self, w, depth_id, position='qkv'):
        ...
        if position == 'qkv':
            self.q_lora_W[depth_id] = w.chunk(3, dim=1)[0]
            self.k_lora_W[depth_id] = w.chunk(3, dim=1)[1]
            self.v_lora_W[depth_id] = w.chunk(3, dim=1)[2]
