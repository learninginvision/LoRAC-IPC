...
class CompILoraPool(nn.Module):
    ...
    def create_parameters(self):
        ...
        for attr_name in self.attributes:
            ...
            if attr_name in ['q_lora', 'k_lora', 'v_lora', 'out_lora']:
                ...
                self.register_buffer(attr_name+'_IS', torch.zeros((self.pool_size, self.depth, 1, 1)))
                self.register_buffer(attr_name+'_ISB', torch.zeros((self.pool_size, self.depth, 1, 1)))
                ...
                # self.register_buffer(attr_name+'_IS', torch.zeros((self.depth, 1, 1)))
                self.register_buffer(attr_name+'_IS', torch.zeros((self.pool_size, self.depth, 1, 1), requires_grad=False))
                self.register_buffer(attr_name+'_ISB', torch.zeros((self.pool_size, self.depth, 1, 1), requires_grad=False))
                self.register_buffer(attr_name+'_OB', torch.zeros((self.pool_size, self.pool_size, self.depth), requires_grad=False))
    ...
    def to_device(self, device):
        # attributes = ['q_lora', 'k_lora', 'v_lora', 'out_lora', 'fc1_lora', 'fc2_lora']
        for attr_name in self.attributes:
            for end in ['_A', '_B', '_O', '_OB', '_IS', '_M']:
                param = getattr(self, attr_name+end)
                if param is None:
                    print(attr_name+end)
        ...
        if train:
            ...
            if position == 'qkv':
                ...
                if task_id > 0:
                    ...
                    q = q_his_with_omegas + (self.q_lora_A[task_id, depth_id] @ self.q_lora_B[task_id, depth_id] * self.q_lora_M[task_id - 1, depth_id].reshape(-1, 1))
                    k = k_his_with_omegas + (self.k_lora_A[task_id, depth_id] @ self.k_lora_B[task_id, depth_id] * self.k_lora_M[task_id - 1, depth_id].reshape(-1, 1))
                    v = v_his_with_omegas + (self.v_lora_A[task_id, depth_id] @ self.v_lora_B[task_id, depth_id] * self.v_lora_M[task_id - 1, depth_id].reshape(-1, 1))
    ...
    def calculate_score(self, name, task_id=None, metric="ipt"):
        if metric == "ipt":
            if '_O' in name:
                ipt_score = (self.exp_avg_ipt[name][task_id, :task_id] * self.exp_avg_unc[name][task_id, :task_id]).sum(dim=0)
            # Combine the senstivity and uncertainty 
            else:
                ipt_score = self.exp_avg_ipt[name][task_id] * self.exp_avg_unc[name][task_id]
        else:
        ...
        for attr_name in self.attributes:
            ...
            if cond:
                for end in ['_A', '_B']:
                    ...
                    with torch.no_grad():                     
                        self.ipt[name][task_id] = (param[task_id] * param.grad[task_id]).abs().detach()
                        # Update sensitivity 
                        self.exp_avg_ipt[name][task_id] = self.beta1 * self.exp_avg_ipt[name][task_id] + (1 - self.beta1) * self.ipt[name][task_id]
                        # Update uncertainty 
                        self.exp_avg_unc[name][task_id] = self.beta2 * self.exp_avg_unc[name][task_id] + (1 - self.beta2) * (self.ipt[name][task_id] - self.exp_avg_ipt[name][task_id]).abs()
    ...
    def mask_ipt_lora(self, task_id, device=None, args=None):
        ...
        if task_id < self.pool_size:
            ...
            for attr_name in self.attributes:
                ...
                lora_mask = getattr(self, attr_name+'_M')
                if cond:
                    lora_A_ipt_score = self.calculate_score(name=attr_name+'_A', task_id=task_id, metric="ipt") # (depth, in_dim, r)
                    lora_B_ipt_score = self.calculate_score(name=attr_name+'_B', task_id=task_id, metric="ipt") # (depth, r, out_dim)
                                       
                    lora_ipt_score = lora_A_ipt_score.mean(dim=(1, 2), keepdim=True) + lora_B_ipt_score.mean(dim=(1, 2), keepdim=True)
