...
class CompILoraPool(nn.Module):
    ...
    def cal_delta_w(self, device, task_id=-1, depth_id=-1, train=False, position='qkv'):
        ...
        if train:
            ...
            if position == 'qkv':
                ...
                if task_id > 0:    
                    q = q_his_with_omegas + self.q_lora_W[depth_id] * self.q_lora_M[task_id - 1, depth_id].reshape(-1, 1)
                    k = k_his_with_omegas + self.k_lora_W[depth_id] * self.k_lora_M[task_id - 1, depth_id].reshape(-1, 1)
                    v = v_his_with_omegas + self.v_lora_W[depth_id] * self.v_lora_M[task_id - 1, depth_id].reshape(-1, 1)
                else:
                    q = q_his_with_omegas + self.q_lora_W[depth_id]
                    k = k_his_with_omegas + self.k_lora_W[depth_id]
                    v = q_his_with_omegas + self.v_lora_W[depth_id]
                ...
                if task_id > 0:
                    w = (out_his_with_omegas + self.out_lora_W[depth_id] * self.out_lora_M[task_id - 1, depth_id].reshape(-1, 1)) * self.scaling
                else:
                ...
                if task_id > 0:
                    w = (fc1_his_with_omegas + (self.fc1_lora_W[depth_id] * self.fc1_lora_M[task_id - 1, depth_id].reshape(-1, 1))) * self.scaling
                else:
                ...
                if task_id > 0:    
                    w = (fc2_his_with_omegas + (self.fc2_lora_W[depth_id] * self.fc2_lora_M[task_id - 1, depth_id].reshape(-1, 1))) * self.scaling
                else:
    ...
    def mask_ipt_lora(self, task_id, device=None, args=None):
        ...
        if task_id < self.pool_size:
            ...
            print("number of chosen pos to drop:", int(total_num * args.threshold))
