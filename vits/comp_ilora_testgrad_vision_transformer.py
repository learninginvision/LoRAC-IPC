...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.w_qkv = torch.zeros(dim, dim * 3)
        self.w_q = torch.zeros(dim, dim)
        self.w_k = torch.zeros(dim, dim)
        self.w_v = torch.zeros(dim, dim)
        self.w_out = torch.zeros(dim, dim)
    ...
    def forward(self, x, **kwargs):
        ...
        if 'lora' in kwargs:
            ...
            # self.w_k.retain_grad()
            # self.w_v.retain_grad()
            w = self.qkv.weight.t() + lora_w
            if kwargs['train']:
                self.w_qkv = w
                self.w_qkv.retain_grad()
            qkv = torch.einsum('bnc, cd -> bnd', x, w) + self.qkv.bias
            # q = torch.einsum('bnc, cd -> bnd', x, w_q) + self.qkv.bias.chunk(3, dim=0)[0]
            # k = torch.einsum('bnc, cd -> bnd', x, w_k) + self.qkv.bias.chunk(3, dim=0)[1]
            # v = torch.einsum('bnc, cd -> bnd', x, w_v) + self.qkv.bias.chunk(3, dim=0)[2]
        ...
        if 'lora' in kwargs:
            ...
            w = self.proj.weight.t() + lora_w
            if kwargs['train']:
                self.w_out = w
                self.w_out.retain_grad()
            x = torch.einsum('bnc, cd -> bnd', x, w) + self.proj.bias
...
class Mlp(nn.Module):
    ...
    def forward(self, x, **kwargs):
        if 'lora' in kwargs:
            lora_w = kwargs['lora'].cal_delta_w(device=x.device, task_id=kwargs['task_id'], depth_id=kwargs['depth_id'], train=kwargs['train'], position='fc1')
            w = self.fc1.weight.t() + lora_w
            if kwargs['train']:
                self.fc1_w = w
                self.fc1_w.retain_grad()
            x = torch.einsum('bnc, cd -> bnd', x, w) + self.fc1.bias
        ...
        x = self.drop1(x)
        if 'lora' in kwargs:
            lora_w = kwargs['lora'].cal_delta_w(device=x.device, task_id=kwargs['task_id'], depth_id=kwargs['depth_id'], train=kwargs['train'], position='fc2')
            w = self.fc2.weight.t() + lora_w
            if kwargs['train']:
                self.fc2_w = w
                self.fc2_w.retain_grad()
            x = torch.einsum('bnc, cd -> bnd', x, w) + self.fc2.bias
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            rank=4, lora_pool_size=10, attn=Attention, lora_pool=CompILoraPool, lora_depth=12,
            mlp_structure=[], projection_hidden=None, lora_qkv=[False, False, True], lora_out=False, lora_fc1=False, lora_fc2=False, lora_position='qkv',
            target_rank=4, init_warmup=500, final_warmup=1500, mask_interval=10, beta1=0.85, beta2=0.85, total_step=3000, target_total_rank=None):
        """
        Args:
        ...
        if lora:
            self.lora_layer = lora_pool(pool_size=lora_pool_size, dim=embed_dim, rank=rank, depth=lora_depth, 
                                        beta1=beta1, beta2=beta2,                                             
                                        lora_qkv=lora_qkv, lora_out=lora_out, lora_fc1=lora_fc1, lora_fc2=lora_fc2)
        else:
    ...
    def after_task(self, task_id=-1, device=None, args=None):

        self.lora_layer.after_task(task_id, device, args)
