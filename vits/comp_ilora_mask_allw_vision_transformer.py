...
class Mlp(nn.Module):
    ...
    def forward(self, x, **kwargs):
        if 'lora' in kwargs:
            ...
            w_fc1 = self.fc1.weight.t() + lora_w
            if kwargs['train']:
                kwargs['lora'].get_all_w(w_fc1, depth_id=kwargs['depth_id'], position='fc1')
            x = torch.einsum('bnc, cd -> bnd', x, w_fc1) + self.fc1.bias
        ...
        if 'lora' in kwargs:
            ...
            w_fc2 = self.fc2.weight.t() + lora_w
            if kwargs['train']:
                kwargs['lora'].get_all_w(w_fc2, depth_id=kwargs['depth_id'], position='fc2')
            x = torch.einsum('bnc, cd -> bnd', x, w_fc2) + self.fc2.bias
        else:
            x = self.fc2(x)
        x = self.drop2(x)
        return x
