...
from peft.lora.comp_lora import CompLoraPool
from peft.lora.comp_adalora import CompAdaLoraPool
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            rank=4, lora_pool_size=10, attn=Attention, lora_pool = CompLoraPool, lora_depth=12,
        ...
        if lora:
            if lora_type == 'compada':
                self.lora_layer = lora_pool(pool_size=lora_pool_size, dim=embed_dim, rank=rank, depth=lora_depth, 
                                            
                                            target_rank=target_rank, init_warmup=init_warmup, final_warmup=final_warmup, 
                                            mask_interval=mask_interval, beta1=beta1, beta2=beta2, 
                                            total_step=total_step, target_total_rank=target_total_rank,
    ...
    def get_similarity_loss(self, task_id, device=None, args=None, dtype=None):
        return args.lora_reg * self.lora_layer.loss_similarity(task_id=task_id, device=device, args=args, dtype=dtype)
